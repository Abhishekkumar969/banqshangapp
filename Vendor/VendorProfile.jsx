...
const VendorProfile = () => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user && user.email) {
        try {
          ...
          else console.warn("❌ No vendor record found for this user.");
        } catch (error) {
  ...
  const handleAddItem = async () => {
    if (!itemName.trim() || !vendor) return;

    try {
      const vendorRef = doc(db, "usersAccess", vendor.id);
      ...
      await updateDoc(vendorRef, { items: newItems });
      setItems(newItems);
      setItemName("");
    } catch (error) {
  ...
  const handleUpdateField = async (field) => {
    if (!editValue.trim() || !vendor) return;
    try {
      const vendorRef = doc(db, "usersAccess", vendor.id);
      await updateDoc(vendorRef, { [field]: editValue });
      setVendor({ ...vendor, [field]: editValue });
      setEditField(null);
      setEditValue("");
    } catch (error) {
  ...
  const handleDeleteField = async (field) => {
    ...
    try {
      const vendorRef = doc(db, "usersAccess", vendor.id);
      await updateDoc(vendorRef, { [field]: "" });
      setVendor({ ...vendor, [field]: "" });
    } catch (error) {
  ...
  const handleAddFunctionType = async () => {
    if (!newFunctionType.trim() || !vendor) return;
    ...
    try {
      const vendorRef = doc(db, "usersAccess", vendor.id);
      await updateDoc(vendorRef, { functionTypes: updatedFunctionTypes.filter(ft => ft !== "Default"), items: updatedItems });
        ...
        {vendor ? (
          <>
            ...
            <h2 className={styles.heading}>
              Hi {vendor?.name ? vendor.name : vendor?.firmName ? vendor.firmName : "Vendor"} 👋
            </h2>

            {/* Vendor Email Below the Heading */}
                            ...
                            <>
                              ...
                              <button type="button"
                                onClick={async () => {
                                  ...
                                  const vendorRef = doc(db, "usersAccess", vendor.id);
                                  await updateDoc(vendorRef, { functionTypes: updatedFunctions.filter(f => f !== "Default"), items: updatedItems });
                ...
                .map(fn => (
                  <AccordionItem
                    ...
                    onAddItem={(newItem) => {
                      ...
                      const vendorRef = doc(db, "usersAccess", vendor.id);
                      updateDoc(vendorRef, { items: updatedItems });
                      setItems(updatedItems);
                    }}
                  />
                ))
          ...
          <div className={styles.container}>
            {loading ? (
              ...
              <p className={styles.message}>Pls Login With Vendor profile...</p>
            ) : (
              <div>
                {/* Your Vendor content goes here */}
